...
Опционально можно задать **кодовое слово** (bcrypt), срок жизни ссылки и число просмотров в пределах тарифа.
...
- У залогиненного пользователя при `create` заполняется `secrets.creator_user_id` (для будущих Pro-уведомлений)
