...
3. При первом переходе по ссылке получатель видит секрет, и данные **удаляются из базы** после исчерпания лимита просмотров (по умолчанию — один раз). Повторное открытие показывает, что ссылка недействительна.

> **Кодовое слово на ссылку** — запланировано на следующий этап; в текущей версии поле в форме отключено.
